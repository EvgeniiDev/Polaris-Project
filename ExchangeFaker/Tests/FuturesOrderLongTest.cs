...
namespace ExchangeFaker.Tests
{
    ...
    public class FuturesOrderLongTest
    {
        ...
        public void BuySomeCoinsWhenPriceHasNotReached()
        {
            ...
            var account = exchange.CreateAccount("юджин", tickerName, startBalance);
            var buy = new List<Order>() { new Order(10, 100) };
            var order = account.PostFuturesOrder(DataObjects.Long, "шоколадные монетки", 1, buy);
        ...
        public void BuySomeCoinsAndSellAllByTakes()
        {
            ...
            var order = account.PostFuturesOrder(DataObjects.Long, tickerName, leverage, buy, take);
        ...
        public void BuySomeCoinsAndSellPartByStop()
        {
            ...
            var order = account.PostFuturesOrder(DataObjects.Long, tickerName, leverage, buy, null, stop);
            var priceGoals = new List<decimal>() { 9m, 10m, 11m, 8m };
            MovePrice(priceGoals, tickerName);
            Assert.AreEqual(Status.Open, account.Orders[order].Status);
            Assert.AreEqual(75m, account.Orders[order].Amount);
        ...
        public void BuySomeCoinsAndSellAllByTakeAndStop()
        {
            ...
            var stop = new List<Order>() { new Order(9, 25) };
            var order = account.PostFuturesOrder(DataObjects.Long, tickerName, leverage, buy, take, stop);
        ...
        public void BuySomeCoinsWhenHasMoneyOnlyOnPartOfCoins()
        {
            ...
            Assert.Catch(
                ...
                {
                    var account = exchange.CreateAccount("юджин", tickerName, startBalance);
                    var order = account.PostFuturesOrder(DataObjects.Long, tickerName, leverage, buy);
                });
            var priceGoals = new List<decimal>() { 9m, 10m, 11m, 8m };
            MovePrice(priceGoals, tickerName);
        }
        ...
        public void BuyCoinsAndGetLiquidation()
        {
            ...
            var buy = new List<Order>() { new Order(10, 100) };
            var order = account.PostFuturesOrder(DataObjects.Long, "шоколадные монетки", 10, buy, null, null);
